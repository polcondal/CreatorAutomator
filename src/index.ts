...
import { Account, AccountStatus } from './models/Account.js';
...
const baseURL: string = 'https://www.bing.com/images/create/';

// All generated images start with this prefix
// https://th.bing.com/th/id/

// This starts as undefined in order to only check once
let collectionExists: boolean | undefined = undefined;
let collectionSelected = false;


// Get the accounts from the config
const accounts: Account[] = JsonUtils.ImportAccountsFromJson() as Account[];
const prompt: string = JsonUtils.GetPrompt() as string;
const collection: string = JsonUtils.GetCollection() as string;
...
async function AccountManager(browser: Browser, accounts: Account[]) {
...
// Bot Execution
async function RunAccount(browser: Browser, account: Account, downloadImages: boolean = false) {
    ...
    try {
        ...
        const passwordForm: Locator = page.getByPlaceholder("Password");
    ...
    // Fill in prompt
    const promptForm: Locator = page.getByPlaceholder("Want to see how Image Creator works? Select Surprise Me, then Create");
    ...
    // Status Variables --------------
    let imgBlockVisible: boolean = false;
    let loadingBarVisible: boolean = false;
    let createButtonAvailable: boolean = false;

    let accountBoosted: boolean = false;

    let createButton: Locator | undefined = undefined;
    // -------------------------------
    ...
    async function UpdateState(): Promise<void> {
        ...
        const boostBalance: string = (await boostBalanceLocator.getAttribute("aria-label")) as string;
            ...
            // If button is disabled or not found, it either means it's generating / or the user has been hard throttled
                ...
                while (!imgBlockVisible) {
                    ...
                    if (imgBlockVisible) {
                        ...
                        const imagesAmount: number = await GetAmountOfGeneratedImagesFromGridGiric(page.locator("#gir_async")) as number;
 ...
 */
async function saveImagesToCollection(amount: number, page: Page): Promise<void> {
    ...
    for (let i = 0; i < amount; i++) {
        ...
        const saveButtonLocator: Locator = page.locator("#svctrlbtn");
        ...
        const listBoxLocator: Locator = page.getByRole("listbox", { name: "Your Collections" });

        // Get all the list elements from the listbox as locators
        const listBoxItems: Locator[] = await listBoxLocator.locator("li").all();
        ...
        if (collectionSelected == false) {
            ...
            for (let item of listBoxItems) {
                const name: string = await item.locator("div").nth(1).innerText();
                const isItemSelected: boolean = (await item.getAttribute("aria-selected")) === "true";
 ...
 */
async function CreateCollectionIfNotExists(page: Page, listBoxItems: Locator[]): Promise<void> {
    ...
    const createCollectionButton: Locator = page.locator("#svctrlhvrbtn");
 ...
 * @param page The page to operate on
 */
async function saveImagesToDisk(amount: number, page: Page): Promise<void> {
    ...
    const imageTest: Locator[] = await page.locator(`img[alt="${prompt}"]`).all();
    ...
    for (let i = 0; i < amount; i++) {
        ...
        const imageChild: Locator = imageTest[i];
        logger.trace("Getting image child = " + JSON.stringify(imageChild));


        // Get the src attribute of the image
        let imgLink: string | undefined = await imageChild.getAttribute("src") ?? undefined;
        ...
        const responseBuffer: Buffer = await imgResp.body();
 ...
 * @returns
 */
async function checkForThrottling(account: Account, page: Page, accountboosted: boolean): Promise<void> {
 ...
 */
async function isHardThrottled(page: Page): Promise<boolean> {
    const throttlingTopRightMsg: Locator = page.locator("#gilen_son");
    const locatorClassString: string = await throttlingTopRightMsg.getAttribute("class") as string;
 ...
 */
async function isSoftThrottled(page: Page): Promise<boolean> {
    const softThrottleMsg: Locator = page.locator("#gilen_c");
    const locatorClassString: string = await softThrottleMsg.getAttribute("class") as string;
 ...
 */
async function GetAmountOfGeneratedImagesFromGridGiric(giric: Locator): Promise<number> {
    const locatorClass = await giric.getAttribute("class") as string;
