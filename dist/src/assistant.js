...
const proceed = await Utils.getBoolInput("Proceed? (y/n) \n");
...
async function mainMenu() {
    ...
    while (!exit) {
        ...
        const downloadImages = JsonUtils.GetDownloadImages();
        ...
        const option = await Utils.getNumberInput("Choose an option: ");
        switch (option) {
            case 1:
                await AccountsMenu();
                break;
            case 2:
                const newPrompt = await Utils.getStringInput("Enter the new prompt (leave blank for no change): \n");
                if (!newPrompt) {
                    break;
                }
                ...
                break;
            case 3:
                const downloadImages = await Utils.getBoolInput("Download newly generated images instead of saving them to a collection? (y/n) \n");
                AssitantUtils.SetDownloadImages(downloadImages);
                break;
            case 4:
                const newCollection = await Utils.getStringInput("Enter the name of the collection to save images in (it wont be used unless downloadImages is True) \n");
 ...
 */
async function AccountsMenu(accounts = JsonUtils.ImportAccountsFromJson()) {
